...
The user has disabled per-phase teaching blocks. Wherever this protocol says **"Include the teaching summary"** or shows a "Teaching Notes" / "What you learned / Important words / Why this phase matters / What happens next" block — skip it entirely. Save the artifact, present a brief summary if useful, ask for approval, move on. Teaching is on-demand only via `/teach`, `/teach-implement`, `/reflect`, `/interview`.
